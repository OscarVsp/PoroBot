...
class PatchNote:
    ...
    def __init__(self, previous: int = 0, lang: str = "en-gb"):
        ...
        try:
            ...
            logging.error(
                f"An error occured during the request of the patchnote data at url '{self.patch_request_urlquest_url}'"
            )
            raise
        ...
        try:
            [self.season_number, self.patch_number] = (
                patch_note_data["result"]["data"]["all"]["nodes"][0]["url"]["url"].split("/")[3].split("-")[1:3]
            )
        except Exception:
            [self.season_number, self.patch_number] = [0, 0]
            logging.warn(
                f"Unable to found season_number and patch_number from the patchnote data. Placeholder values used instead."
            )

        soup = BeautifulSoup(
            patch_note_data["result"]["data"]["all"]["nodes"][0]["patch_notes_body"][0]["patch_notes"]["html"],
            "html.parser",
        )

        try:
            self.description: str = (
                markdownify(str(soup.blockquote), heading_style="ATX").replace(">", "").strip().replace("\n \n", "\n")
            )
        except Exception:
