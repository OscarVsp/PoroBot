...
class MemberSelectionView(ConfirmationView):
    ...
    def embed(self) -> disnake.Embed:
        ...
        embed.add_field(
            ...
            value="\n".join(f"> **{member.display_name}**" for member in self.selected_members)
            if len(self.selected_members)
    ...
    def refresh_selection(self):
            ...
            if member in self.selected_members:
                if len(self.remove.options) < 25:
                    self.remove.options.append(
                        disnake.SelectOption(label=member.display_name, emoji="🗑️", value=str(member.id))
                    )
                else:
                    self.options_limited = True
            elif self.check(
                member=member, selected_members=self.selected_members, original_interaction=self.target, size=self.size
            ):
                if len(self.add.options) < 25:
                    self.add.options.append(
                        disnake.SelectOption(label=member.display_name, emoji=Emotes.ADD_FRIEND, value=str(member.id))
                    )
                else:
                    self.options_limited = True
        ...
        self.role.options = [
            disnake.SelectOption(label=role.name, emoji=Emotes.ADD_FRIEND, value=str(role.id))
            for role in self.target.guild.roles
        ]
        ...
        if self.size:
            self.confirm.disabled = (
                not len(self.selected_members) in self.size
                if isinstance(self.size, list)
                else len(self.selected_members) != self.size
            )
    ...
    @disnake.ui.select(
        ...
        max_values=1,
        row=2,
        placeholder="🗑️ Retirer des membres",
    ...
    async def role(self, select: disnake.ui.Select, interaction: disnake.MessageInteraction):
        for role_id in select.values:
            ...
            for member in role.members:
                if member not in self.selected_members and self.check(
                    member=member,
                    selected_members=self.selected_members,
                    original_interaction=self.target,
                    size=self.size,
                ):
                    self.selected_members.append(member)
                    self.remove.disabled = False
