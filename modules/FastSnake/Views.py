...
async def QCM(
    ...
    --------
        `QCMReturnData`
    """
    return QCMReturnData(
        await process(QCMView(target, embeds, title, description, timeout, color, choices, pre_selection))
    )
...
async def QRM(
    ...
    --------
        `QRMReturnData`
    """
    return QRMReturnData(
        await process(QRMView(target, embeds, title, description, timeout, color, choices, pre_selection, min, max))
    )
...
async def Selection(
    ...
    --------
        `QRMReturnData`
    """
    return SelectionViewReturnData(
        await process(SelectionView(target, embeds, title, description, timeout, options, color, thumbnail))
    )
