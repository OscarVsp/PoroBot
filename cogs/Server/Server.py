...
class Server(commands.Cog):
    ...
    async def lock(self, channel : disnake.VoiceChannel):
        authorized_role = await channel.guild.create_role(name=f"{channel.name} authorized",reason="Unlock channel")
        ...
        for member in channel.members:
            if member.voice.suppress:
                ...
                await member.send(embed=FastEmbed(
                    ...
                    description="Le channel vocal dans lequel tu te trouves vient d'être déverrouillé ! Tu peux à nouveau parler normalement."      
                ))  
    ...
    async def on_connect_to_locked_channel(self, member : disnake.Member, channel : disnake.VoiceChannel):
        if not self.is_authorized(member, channel):
            await member.send(embed=FastEmbed(
                    ...
                    description="Le channel vocal que tu viens de rejoindre est vérrouillé. Cela veut dire que tu ne peux pas parler, mais tu peux toujours écouter les autres et regarder des streams.\nJe te previendrais quand le channel sera déverrouillé."
                ))  
