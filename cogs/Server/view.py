...
class Locker(disnake.ui.View):
    ...
    def notification_embed(self) -> disnake.Embed:
        ...
        return FS.Embed(title=self.title, description=description)
    ...
    async def lock(self, inter: disnake.CommandInteraction):
        ...
        self.authorized_role = await self.channel.guild.create_role(
            name=f"{self.channel.id} {Locker.role_name_authorized}", reason=f"Lock channel - {self.reason}"
        )
        self.unauthorized_role = await self.channel.guild.create_role(
            name=f"{self.channel.id} {Locker.role_name_unauthorized}", reason=f"Lock channel - {self.reason}"
        )
    ...
    def refresh_presence(self):
        if len(self.unauthorized_role.members) > 0:
            self.authorize.options = [
                disnake.SelectOption(label=f"{member.display_name}", emoji=FS.Emotes.ADD_FRIEND, value=str(member.id))
                for member in self.unauthorized_role.members
            ]
        ...
        if len(self.authorized_role.members) > 0:
            self.unauthorize.options = [
                disnake.SelectOption(label=f"{member.display_name}", emoji="🗑️", value=str(member.id))
                for member in self.authorized_role.members
            ]
    ...
    async def unlock(self, inter: disnake.MessageInteraction = None):
        ...
        await inter.edit_original_message(
            embed=FS.Embed(
                ...
                description=f"{FS.Assets.Emotes.LOADING} Déverrouillage en cours... ⌛",
            ),
            view=None,
        )
        self.stop()
    ...
    @disnake.ui.select(
        ...
        max_values=1,
        row=2,
        placeholder="🚫 Restreindre des participants",
        options=[disnake.SelectOption(label="placeholder", value="1")],
    )
    ...
    @disnake.ui.select(
        ...
        max_values=1,
        row=3,
        placeholder="✅ Autoriser des spectateurs",
        options=[disnake.SelectOption(label="placeholder", value="1")],
    )
