...
class PhaseCreationView(ConfirmationView):
    ...
    def __init__(self, target: Target, embeds : List[disnake.Embed], phase_idx : int):
        super().__init__(target, embeds, f"{FS.Emotes.BRACKET} __**CRÉATION DE LA PHASE {phase_idx+1}**__", "Configure la phase avec les options ci-dessous.", 300)
    ...
    def embed(self) -> disnake.Embed:
        ...
        embed.description += "\n**Taille des groupes :** "+(f"`{self.taille_groups}`" if self.taille_groups else "`N/A`")
        return embed
    ...
    async def update(self, inter: disnake.MessageInteraction):
        if self.format_text:
            if self.format_text == Tournament2v2Roll.TYPE and self.size.disabled:
                ...
                self.size.options = [disnake.SelectOption(label=" joueurs", value=str(i), emoji=FS.Emotes.Num(i),description=f"Pour un total de {i-1} rounds") for i in [4,5,8]]
                self.size.disabled = False
        return await super().update(inter)
            
        
    @disnake.ui.select(placeholder="Format de la phase",options=[disnake.SelectOption(label=Tournament2v2Roll.TYPE, emoji="🔀",description="Robin round par équipes de deux qui changent entre chaque round.")])
    ...
    @disnake.ui.select(placeholder="Nombre de groupe",options=[disnake.SelectOption(label=f"{i+1}", value=str(i+1),description=f"{i+1} groupe"+("s" if i>0 else "") +" en parrallèle") for i in range(25)])
    ...
    @disnake.ui.select(placeholder="Taille des groupe (sélectionne d'abord le format)",options=[disnake.SelectOption(label="Placeholder")])
        ...
        if self.is_ended:
            for i in range(phaseCreationView.nombre_group):
                if phaseCreationView.format_text == Tournament2v2Roll.TYPE:
                    self.tournaments.append(Tournament2v2Roll(phaseCreationView.target.guild,phaseCreationView.taille_groups,f"PHASE {phaseCreationView.phase_idx+1}" + (f" - GROUPE {chr(ord('A') +i)}" if phaseCreationView.nombre_group > 1 else "")))
        
        
    
async def phaseCreation(inter : disnake.ApplicationCommandInteraction, phase_idx : int, embeds : List[disnake.Embed]) -> PhaseCreationReturnData:
