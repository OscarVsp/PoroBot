...
class Score:
    ...
    def options(self) -> List[disnake.SelectOption]:
        return [
            disnake.SelectOption(label=self.name, emoji=self.emoji, value=f"{self.id}{i}") for i in range(self.per_team)
        ]
...
class Team:
    ...
    def add_scores(self, values: List[int]):
        if len(self._scores) == len(values):
            ...
            logging.error(
                f"{self.log_id} Error on adding scores : list of values is not the same size than the number of score ({len(self._scores)})."
            )
    ...
    def remove_scores(self, values: List[int]) -> None:
        if len(self._scores) == len(values):
            ...
            logging.error(
                f"{self.log_id} Error on removing scores : list of values is not the same size than the number of score ({len(self._scores)})."
            )
    ...
    def set_scores(self, values: List[int]) -> None:
        if len(self._scores) == len(values):
            ...
            logging.error(
                f"{self.log_id} Error on setting scores : list of values is not the same size than the number of score ({len(self._scores)})."
            )
    ...
    async def move_to(self, channel: VocalGuildChannel = None, channels: List[VocalGuildChannel] = None):
        if channel and not channels:
            ...
            if len(channels) == len(self._entities):
                ...
                logging.error(
                    f"{self.log_id} Error on moving entities : size of 'channels' is not compatible with the number of entities"
                )
        elif channel and channels:
            logging.error(
                f"{self.log_id} Error on moving entities : Only one of 'channel' or 'channels' should be used."
            )
        else:

            logging.error(
                f"{self.log_id} Error on moving entities : 'channel' and 'channels' are both None. One has to be set.'"
            )
...
class Round(Container):
    ...
    def embed(self) -> disnake.Embed:
        return FS.Embed(title=self.title, color=self.embed_color, fields=[m.field for m in self._matches])

    @property
    def embed_detailled(self) -> disnake.Embed:
        return FS.Embed(title=self.title, color=self.embed_color, fields=[m.field_detailled for m in self._matches])
    ...
    def remove_score(
        self, match: Union[Match, int], entity: Union[Entity, int], value: int = 1, index: int = 0
    ) -> None:
    ...
    async def move_to(self, channel: VocalGuildChannel = None, channels: List[VocalGuildChannel] = None):
        if channel and not channels:
            ...
            if len(channels) == len(self._matches):
                ...
                logging.error(
                    f"{self.log_id} Error on moving matches : size of 'channels' is not compatible with the number of matches."
                )
        elif channel and channels:
            logging.error(
                f"{self.log_id} Error on moving matches : Only one of 'channel' or 'channels' should be used."
            )
        else:

            logging.error(
                f"{self.log_id} Error on moving matches : 'channel' and 'channels' are both None. One has to be set.'"
            )
...
class TournamentData(Container):
    ...
    @staticmethod
    def rank(sorted_players: List[Player]) -> List[int]:
        ranks: List[int] = []
    ...
    @classmethod
    def rank_emotes(cls, sorted_players: List[Player]) -> List[str]:
        return [FS.Emotes.Ranks(rank) for rank in cls.rank(sorted_players)]
    ...
    def current_round(self) -> Optional[Round]:
        for round in self._rounds:
            if round.state == State.SET or round.state == State.STARTED:
                return round
        return round
    ...
    def clear_score(
        self, round: Union[Round, int], match: Union[Match, int], entity: Union[Entity, int], index: int = 0
    ) -> None:
        ...
        for round in self.last_state.get("rounds"):
            for match in round.get("entities"):
                for team in match.get("entities"):
                    ...
                    if (
                        players[0].get("id")
                        == self.rounds[round.get("round_idx")]
                        .matches[match.get("match_idx")]
                        .teams[team.get("team_idx")]
                        .players[0]
                        .id
                        and players[1].get("id")
                        == self.rounds[round.get("round_idx")]
                        .matches[match.get("match_idx")]
                        .teams[team.get("team_idx")]
                        .players[1]
                        ...
                        self.set_scores(
                            round.get("round_idx"), match.get("match_idx"), team.get("team_idx"), team.get("scores")
                        )
