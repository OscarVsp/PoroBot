...
class AdminView(disnake.ui.View):
    ...
    def refresh_options(self) -> None:
        if self.match_selected and self.round_selected:
            self.match_selection.placeholder = f"{self.round_selected.round_idx+1}{chr(ord('A') + self.match_selected.match_idx)} : {self.match_selected.teams[0].name} VS {self.match_selected.teams[1].name}"
            self.set_team_1_score.disabled = False
            self.set_team_1_score.placeholder = (
                f"{self.match_selected.teams[0].name} : {self.match_selected.teams[0].scores_description}"
            )
            self.set_team_2_score.disabled = False
            self.set_team_2_score.placeholder = (
                f"{self.match_selected.teams[1].name} : {self.match_selected.teams[1].scores_description}"
            )
        else:
...
class DraftManager:
    ...
    async def start(self) -> "DraftManager":
        for channel in self.channels:
            self.messages[channel] = await channel.send(embed=self.embed(channel))
        return self
