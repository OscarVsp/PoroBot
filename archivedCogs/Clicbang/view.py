...
class BangMenu(disnake.ui.View):
    ...
    def embed(self):
        ...
        if len(self.players) != 0:
            embed.add_field(name="__Joueurs : __", value="\n".join([p.display_name for p in self.players]))
        else:
            embed.add_field(name="__Joueurs : __", value="*Aucun joueur dans la partie*")
        return embed
...
class BangGame(disnake.ui.View):
    ...
    def embed(self, message: str = ""):
        embed = FS.Embed(title="__**BANG - PARTIE**__")
        if self.lastCarte == None:
            embed.add_field(
                name="__Dernière carte jouée :__", value="*Les cartes jouées s'afficheront ici*", inline=False
            )
            embed.set_thumbnail(url=FS.Images.Cards.BACK)
        else:
            embed.add_field(
                name="__Dernière carte jouée :__", value=f"**{self.lastCarte}**\n*{self.lastCarte.effet}*", inline=False
            )
            embed.set_thumbnail(url=self.lastCarte.image)
        embed.add_field(name="__Score :__", value=f"{self.score_tab}", inline=False)
        if self.looser != None:
            embed.add_field(
    ...
    async def start_game(self, interaction: disnake.MessageInteraction):
        ...
        self.cartes = [
            Carte(number + 1, color)
            for number in range(self.max_value)
            for color in ["pique", "coeur", "carreau", "trefle"]
        ]
        self.lastCarte = None
