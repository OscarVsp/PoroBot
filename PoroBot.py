...
async def on_slash_command_error(interaction: ApplicationCommandInteraction, error: Exception) -> None:
    ...
    await interaction.send(embed=disnake.Embed(title=":x:__**ERROR**__:x:",description=f"Une erreur s'est produite lors de la commande **/{interaction.name}**\n{self.get_user(281401408597655552).mention} a été prévenu et corrigera ce bug au plus vite !\nUtilise `/beer` pour un bière de consolation :beer:").set_thumbnail(url="https://i.imgur.com/U7rBtRu.png"),delete_after=10)
    await bot.get_channel(self.logs_channel).send(embed=disnake.Embed(title=f":x: __** ERROR**__ :x:",description=f"""```{error}```\nRaised on command **/{interaction.name}** from {interaction.channel.mention} by {interaction.author.mention}."""))
    n = (len(tb) // 4096) 
    for i in range(n):
        await bot.get_channel(self.logs_channel).send(embed=disnake.Embed(description=f"```python\n{tb[4096*i:4096*(i+1)]}```"))
    await bot.get_channel(self.logs_channel).send(embed=disnake.Embed(description=f"```python\n{tb[4096*n:]}```"))
    raise error
